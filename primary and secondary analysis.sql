...
-- ------------------------------------------------------------------------------------------------------------------------------------------------
-- CALCULATE THE AVERAGE FARE PER TRIP FOR EACH CITY AND COMPARE IT WITH THE CITY'S AVERAGE
-- TRIP DISTANCE.IDENTIFY THE CITIES WITH THE HIGHEST AND LOWEST AVERAGE FARE PER TRIP TO 
-- ASSESS PRICING EFFICIENCY ACROSS LOCATIONS.
WITH CTE AS
(
...
-- -------------------------------------------------------------------------------------------------------------------------------------------------
-- CALCULATE THE AVERAGE PASSENGER AND DRIVER RATING FOR EACH CITY,SEGMENTED,BY PASSENGER TYPE (NEW VS REPEAT)
...
-- FOR EACH CITY IDENTIFY THE MONTH WITH THE HIGHEST TOTAL TRIPS(PEAK DEMAND) AND THE MONTH WITH LOWEST TRIPS
-- (LOW DEMAND).THIS ANALYSIS WILL HELP GOOD CABS TO UNDERSTAND SEASONAL PATTERNS AND ADJUST RESOURCES
-- ACCORDINGLY.
WITH CTE1 AS
(
WITH CTE AS
...
FROM CTE1;
-- ------------------------------------------------------------------------------------------------------------------------------------------------
-- REPEAT PASSENGER FREQUENCY AND CITY CONTRIBUTIONS ANALYSIS
-- ANALYSE THE FREQUECY OF TRIPS TAKEN BY REPEAT PASSENGERS IN EACH CITY.
-- PERCENT OF REPEAT PASSENGERS TAKING 2 TRIPS,3 TRIPS ETC..........
WITH CTE AS
(
SELECT *,
...
GROUP BY city_name;
-- ------------------------------------------------------------------------------------------------------------------------------------------------
-- MONTHLY TARGET ACHIEVEMENTS ANALSIS OF KEY METRICS
-- FOR EACH CITY,EVALUATE MONTHLY PERFORMANCE AGAINST TARGETS FOR TOTAL TRIPS NEW PASSENGERS,AND AVERAGE PASSENGER
-- CALCULATE THE  PERCENTAGE DIFFERENCE ANY CONSISTENT PATTERNS IN TARGET ACHEIVEMENT PARTICULARLY ACROSS TOURISM
-- VERSUS BUSSINESS FOCUSED COUNTRIES.
...
/* HIGHEST AND LOWEST REPEAT PASSENGER RATE FOR EACH CITY ACROSS THE SIX MONTH PERIOD.IDENTIFY 
THE TOP 2 AND BOTTOM 2 CITIES BASED ON  THEIR RPR TO DETERMINE THE LOCATIONS HAVE THE STRONGEST AND WEAKEST RATES.*/
WITH CTE AS 
(
...
/*
SIMILARLY ANALYSE THE RPR% FOR MONTHS ACROSS ALL CITIES AND IDENTIFY THE MONTHS WITH HIGHEST AND LOWEST RPR ACROSS EACH
CITY
*/
WITH CTE AS
(
